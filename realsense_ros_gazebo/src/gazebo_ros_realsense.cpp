...
void GazeboRosRealsense::OnNewFrame(const rendering::CameraPtr cam,
  ...
  if (camera_id == COLOR_CAMERA_NAME)
  {
    ...
    color_image_msg_ = this->image_msg_;
                                              ...
                                              uint32_t step_arg, void *data_arg)
{
  ...
  sensor_msgs::Image color_source_msg;
  bool use_color_source = false;
  {
    std::lock_guard<std::mutex> lock(color_image_mutex_);
    if (has_color_image_) {
      color_source_msg = color_image_msg_;
      use_color_source = true;
    }
  }

  const sensor_msgs::Image &image_source = use_color_source ? color_source_msg : this->image_msg_;
  const uint8_t *image_src = image_source.data.data();
  const size_t image_size = image_source.data.size();
  const bool has_rgb = image_size == rows_arg * cols_arg * 3;
  const bool has_mono = image_size == rows_arg * cols_arg;
  ...
  for (uint32_t j = 0; j < rows_arg; j++)
  {
    ...
    for (uint32_t i = 0; i < cols_arg; i++, ++iter_x, ++iter_y, ++iter_z, ++iter_rgb)
    {
      ...
      if (has_rgb && image_src)
      {
        // color
        const size_t color_index = (j * cols_arg + i) * 3;
        iter_rgb[0] = image_src[color_index + 0];
        iter_rgb[1] = image_src[color_index + 1];
        iter_rgb[2] = image_src[color_index + 2];
      }
      else if (has_mono && image_src)
      {
        // mono (or bayer?  @todo; fix for bayer)
        const uint8_t intensity = image_src[i + j * cols_arg];
        iter_rgb[0] = intensity;
        iter_rgb[1] = intensity;
        iter_rgb[2] = intensity;
      }
      else
      {
        // no image
